...
def main(mode):
    ...
    specific_dataset_cfg = {
        "config": {
            ...
            "synthetic_data": {
                    "init_proba": 0.2,# begin proba to generate synthetic document
    ...
    specific_model_cfg = {
        ...
        }, # before fine-tuning on real data, we first fine-tune the model on synthetic data only
        # then, we fine-tune the model on real data using the weights of the model fine-tuned on synthetic data via transfer learning
    }

    specific_training_cfg = {
        "output_folder": "daniel_custom_dataset_with_synth",  # folder name for checkpoints and results
    ...
    if params["training_params"]["use_wandb"]:
        init_wandb(projet_name="daniel", exp_id='custom-dataset-fine-tuning-with-synth-data', params=params, dataset_name=dataset_name)
