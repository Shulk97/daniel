...
def main(mode):
    dataset_name = "IAM"
    dataset_level = "page"
    dataset_variant = "_sem-150dpi-pg"

    specific_dataset_cfg = {
        "config": {
            "layout_tokens_mode": 'MULTI',
            "labels_name": "formatted-IAM-DB-subwords-bart.pkl", # was not used at the beginning because of the bug
            "other_samples":'huggingface:Datasets/raw/wiki_cache-ter',
            "mean": [235.74465752],
            "std": [44.38831295],
    ...
    specific_model_cfg = {
        ...
        "transfer_learning": {
                ...
                "encoder": ["encoder", "outputs/daniel_iam_ner_strategy_A_rwth_split/checkpoints/best-IAM_70.pt", True, True],
                "decoder": ["decoder", "outputs/daniel_iam_ner_strategy_A_rwth_split/checkpoints/best-IAM_70.pt", True, False],
            },
    }

    specific_training_cfg = {
        "output_folder": "daniel_iam_strategy_C",  # folder name for checkpoint and results
        ...
        "max_char_prediction": 450, # max number of token prediction during training, it determines the max length in the positional encoding
        "max_char_inference": 250,  # max number of token prediction
    ...
    if params["training_params"]["use_wandb"]:
        init_wandb(projet_name="daniel", exp_id='iam-strat-C', params=params, dataset_name=dataset_name)
