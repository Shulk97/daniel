...
sys.path.append(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(PARENT_DIR))))
)
sys.path.append(
    os.path.dirname(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(PARENT_DIR))))
...
from OCR.document_OCR.daniel.READ.compatible_fonts_read import (
    valid_fonts as read_valid_fonts,
    hw_fonts_wiki,
)
from OCR.document_OCR.daniel.RIMES.compatible_fonts_rimes import (
    valid_fonts as rimes_valid_fonts,
)
from OCR.document_OCR.daniel.iam.compatible_fonts_iam import (
    hw_extended_valid_fonts as valid_hw_extended_iam_fonts,
    printed_valid_fonts as valid_printed_iam_fonts,
)
from OCR.document_OCR.daniel.RIMES.compatible_fonts_rimes import (
    valid_hw_extended_fonts as valid_hw_extended_rimes_fonts,
    valid_printed_fonts as valid_printed_rimes_fonts,
)
...
def main(mode):
    ...
    specific_dataset_cfg = {
        ...
        "valid": {
            "IAM-valid": [
                ("MULTI_LINGUAL", "valid-IAM"),
            ],
            "RIMES-valid": [
                ("MULTI_LINGUAL", "valid-RIMES"),
            ],
            "READ_2016-valid": [
                ("MULTI_LINGUAL", "valid-READ_2016"),
            ],
            "IAM_NER-valid": [
                ("MULTI_LINGUAL", "valid-IAM_NER"),
            ],
        },
        ...
        "config": {
            "tagging_mode": "after",
            "labels_name": "labels-multi-ner-custom-split.pkl",
            "mean": [235.42100521],
            "std": [44.92433027],
            "multi_sampling": True,
            ...
            #     'READ_2016':'Datasets/raw/deu_wikipedia_2007_1M-sentences-normalized.txt'
            # },
            ...
            "synthetic_data": {
                ...
                "max_nb_lines_READ_2016": max_nb_lines[
                    "READ_2016"
                ],  # maximum number of lines for curriculum learning
                "max_nb_lines_IAM": max_nb_lines[
                    "IAM"
                ],  # maximum number of lines for curriculum learning
                "max_nb_lines_IAM_NER": max_nb_lines[
                    "IAM_NER"
                ],  # maximum number of lines for curriculum learning
                "max_nb_lines_RIMES": max_nb_lines[
                    "RIMES"
                ],  # maximum number of lines for curriculum learning
                ...
                "curr_step": int(
                    1
                ),  # interval to increase the number of lines for curriculum learning
    ...
    specific_model_cfg = {
        "models": {"encoder": FCN_Encoder_Widther},
        "use_checkpointing": False,
        "transfer_learning": {
            # model_name: [state_dict_name, checkpoint_path, learnable, strict]
            "encoder": [
                "encoder",
                "outputs/daniel_multi_synth/checkpoints/last_1875.pt",
                True,
                True,
            ],
            "decoder": [
    ...
    specific_training_cfg = {
        "output_folder": "daniel_iam_ner_strategy_A_custom_split",  # folder name for checkpoint and results
        "max_nb_epochs": 50000,  # maximum number of epochs before to stop
        "load_epoch": "best",  # ["best", "last"]: last to continue training, best to evaluate
        ...
        "set_name_focus_metrics": [
            "{}-valid".format(ds_name) for ds_name in dataset_names
    ...
    default_dataset_cfg = default_dataset_config_factory(
        dataset_name, dataset_level, dataset_variant, multi=True
    )
    params["dataset_params"] = merge_config_dicts(
        default_dataset_cfg, specific_dataset_cfg
    )
    params["training_params"] = merge_config_dicts(
        default_training_cfg, specific_training_cfg
    )

    params["dataset_params"] = complete_dataset_params(
        params["dataset_params"], params["model_params"], params["training_params"]
    )

    params["model_params"]["max_char_prediction"] = params["training_params"][
        "max_char_prediction"
    ]
    if params["training_params"]["use_wandb"]:
        init_wandb(
            projet_name="daniel",
            exp_id="strat-A-iam-ner-custom-split",
            params=params,
            dataset_name=dataset_name,
        )

    run(params, mode=mode, dataset_names=["IAM_NER"], set_names=["test"], multi=True)
